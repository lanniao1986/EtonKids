...
public class ColorPickerView extends View {
	...
	private void init() {
		...
		mLeftBitmap = BitmapFactory.decodeResource(mContext.getResources(),
				R.drawable.reading__color_view__button);
		...
		mRightBitmap = BitmapFactory.decodeResource(mContext.getResources(),
				R.drawable.reading__color_view__saturation);
		mRightBitmap2 = BitmapFactory.decodeResource(mContext.getResources(),
				R.drawable.reading__color_view__saturation_press);
	...
	protected void onDraw(Canvas canvas) {
		// 左边
		canvas.drawBitmap(getGradual(), null, new Rect(SPLIT_WIDTH,
				SPLIT_WIDTH, LEFT_WIDTH + SPLIT_WIDTH, mHeight - SPLIT_WIDTH),
				mBitmapPaint);

		// 右边
		mRightColors[1] = mRightPaint.getColor();
		Shader rightShader = new LinearGradient(mWidth - SPLIT_WIDTH
				- RIGHT_WIDTH / 2, SPLIT_WIDTH, mWidth - SPLIT_WIDTH
				- RIGHT_WIDTH / 2, mHeight - SPLIT_WIDTH, mRightColors, null,
				Shader.TileMode.MIRROR);
		mRightPaint.setShader(rightShader);
		canvas.drawRect(new Rect(mWidth - SPLIT_WIDTH - RIGHT_WIDTH,
				SPLIT_WIDTH, mWidth - SPLIT_WIDTH, mHeight - SPLIT_WIDTH),
				mRightPaint);
		...
		if (mRightMove) {
			canvas.drawBitmap(mRightBitmap, mWidth - SPLIT_WIDTH - RIGHT_WIDTH
					- mRightBitmapQuarterWidth, mRightSelectPoint.y
					- mRightBitmapHalfHeight, mBitmapPaint);
		} else {
			canvas.drawBitmap(mRightBitmap2, mWidth - SPLIT_WIDTH - RIGHT_WIDTH
					- mRightBitmapQuarterWidth, mRightSelectPoint.y
					- mRightBitmapHalfHeight, mBitmapPaint);
		}
	}
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_MOVE:
			...
			if (downInLeft) {
				mLeftMove = true;
				proofLeft(x, y);
				mRightPaint.setColor(getLeftColor(mLeftSelectPoint.x
						- SPLIT_WIDTH, mLeftSelectPoint.y - SPLIT_WIDTH));
			} else if (downInRight) {
				mRightMove = true;
				proofRight(x, y);
			}
			...
			if (mCallBackColor == Integer.MAX_VALUE
		...
		case MotionEvent.ACTION_UP:
			...
			invalidate();
			if (mChangedListener != null) {
				mChangedListener.onColorChanged(
						getRightColor(mRightSelectPoint.y - SPLIT_WIDTH),
						mRightPaint.getColor(),
						(mRightSelectPoint.y - SPLIT_WIDTH)
								/ (mHeight - 2 * SPLIT_WIDTH));
			}
	...
	private Bitmap getGradual() {
		if (mGradualChangeBitmap == null) {
			...
			mGradualChangeBitmap = Bitmap.createBitmap(LEFT_WIDTH, mHeight - 2
					* SPLIT_WIDTH, Config.RGB_565);
	...
	private boolean inLeftPanel(float x, float y) {
		if (0 < x && x < SPLIT_WIDTH + LEFT_WIDTH + SPLIT_WIDTH / 2 && 0 < y
				&& y < mWidth) {
			return true;
		} else {
			return false;
	...
	private boolean inRightPanel(float x, float y) {
		if (mWidth - SPLIT_WIDTH - RIGHT_WIDTH - SPLIT_WIDTH / 2 < x
				&& x < mWidth && 0 < y && y < mHeight) {
			return true;
		} else {
			return false;
		}
